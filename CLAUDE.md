...
- `ultra-web.ts` wraps `@ultraos/wallet-sdk` for the web (popup) wallet. **Mainnet only** — there is no testnet Web Wallet (SDK ≥ 0.6.1 rejects testnet with `WEB_WALLET_UNAVAILABLE`, 4302), so `Login.vue` disables the button on every other network and `App.vue` logs a Web Wallet session out on any network change. No event listeners, no `switchNetwork`. `getErrorMessage()` surfaces the SDK's `{ code, message }` rejections.
