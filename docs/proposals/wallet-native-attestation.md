...
Backends that gate features on account balance / status MUST source the account(s) they gate on from the **verified payload** AFTER checking the signature — e.g. the toolkit's UOS-balance gate (§9) reads `payload.account` (the verified active account); a backend that needs the broader set uses the verified `payload.signableAccounts`. **Never** from `ConnectResult.accounts` (unsigned) and **never** from an FE-supplied list in the request body (trivially spoofable). The wallet's signature over the full payload is what makes both `account` and `signableAccounts` trustworthy: an attacker cannot forge a payload listing accounts they don't control without the wallet's signing key.
...
6. `ultra-tool-kit/backend/src/middleware/balance-gate.ts` — new middleware running after `attestation`, before `ratelimit`. When `c.var.identity` is present: read the UOS balance for the verified active account `payload.account` via the W4 `get_balance` tool (a **single** RPC read), refuse with `kind: 'refuse', reason: 'insufficient-uos'` if below the configured threshold (suggest **1 UOS** to start; tune from usage data). When `c.var.identity` is absent (anonymous request) the gate is a no-op — per-IP rate limit is the only defense for unattested users. The balance is cached in-process per (endpoint, active-account) pair for 5 minutes to avoid hammering the chain RPC on every chat turn. Setting `BALANCE_THRESHOLD_UOS` to `0` disables the gate entirely — no RPC read is made and every attested caller passes.
...
**Why gate on the active account only (not the sum across `signableAccounts`).** An Ultra wallet key is frequently an *authority* on many accounts, so for admin/governance keys the attestation enumerates **75–100+** `signableAccounts`. Summing that list fires one `get_currency_balance` RPC per account — 75+ sequential reads per turn — and the public RPC node throttles the burst: reads fail, the gate counts 0, and it issues a false `insufficient-uos` refusal even when the active account is fully funded. This was confirmed live: `ultra.prop1`, 100.94 UOS, 75 `signableAccounts`. Summing is also semantically wrong — the gating balance should be the user's active/selected account, not the aggregate of every account their key can touch. So the gate reads only `payload.account` with a single RPC call. Note that `signableAccounts` remains signature-covered in the payload (still trustworthy, still available to other consumers); the balance gate simply no longer sums it.
